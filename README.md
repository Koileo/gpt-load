...
Environment variables are optional: the admin web UI ("Settings → Codex turn-state watcher") can configure and enable the same watcher. A saved web config takes effect within seconds and overrides the environment; clearing it falls back to the variables. The fields carry the same meaning in both places (bound credential, injection model, healthy/degraded lengths, backup proxy, verification parameters).
