...
Environment variables are optional: the admin web UI ("Settings → Codex turn-state watcher") can configure and enable the same watcher. A saved web config takes effect within seconds and overrides the environment; clearing it falls back to the variables. Web-configured watchers bind automatically to the credential, client model, and upstream model from the first valid observed request, while environment configuration retains its explicit fixed binding for compatibility.
